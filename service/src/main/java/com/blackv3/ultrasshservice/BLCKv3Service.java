...
import com.blackv3.ultrasshservice.aidl.IUltraSSHServiceInternal;
...
import com.blackv3.ultrasshservice.tunnel.TunnelManagerThread;
...
import com.blackv3.ultrasshservice.util.DummyActivity;

public class BLCKv3Service extends Service
		implements SkStatus.StateListener
{
	private static final String TAG = BLCKv3Service.class.getSimpleName();
	public static final String START_SERVICE = "com.blackv3.br:startTunnel";
	...
	public void endTunnelService() {
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				stopSelf();
				SkStatus.removeStateListener(BLCKv3Service.this);
			}
		});
	}
	...
	public static PendingIntent getGraphPendingIntent(Context context) {
		...
		intent.setComponent(new ComponentName(context, context.getPackageName() + ".BLCKv3MainActivity"));
	...
	public static final String TUNNEL_SSH_RESTART_SERVICE = BLCKv3Service.class.getName() + "::restartservicebroadcast",
			TUNNEL_SSH_STOP_SERVICE = BLCKv3Service.class.getName() + "::stopservicebroadcast";
	...
	private void stopdnstt(){
		...
		if (servermodecheck.toLowerCase().contains("127.0.0.1")){
			...
			try{
				...
				intent_stopdnstt.setComponent(new ComponentName("socksrevive.plugin.dragon", "socksrevive.plugin.dragon.DNSTTService"));

				if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
					BLCKv3Service.this.startForegroundService(intent_stopdnstt);
				} else {
